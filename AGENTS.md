  ...
  - **Never embed tool/agent/llm code in source files** - Any XML-like tags (invoke, tool, read, etc) will break PowerShell build
  - **Always test build after source changes** - Run build.ps1 and verify pcmon.ps1 runs without errors
  - **Never use regex for process group matching** - Use exact name arrays with -contains
  - **Keep pages/sec simple** - Don't add complex fallback logic inside hash tables
  - **Clear CommandLines cache** - Always clear $script:CommandLines before repopulating
  - **Build output sync** - Don't edit pcmon.ps1 or dist/index.html - edit src/ and rebuild
  - **Empty catch blocks forbidden** - Always log errors at minimum
  - **Validate all inputs** - Snapshot IDs, process IDs, HTTP methods
  - **Never use $pid variable** - PowerShell treats $pid as read-only. Use $procId or $processId instead
  - **Always define variables before use in timers** - Register-ObjectEvent timers reference variables that must be declared before the timer setup
  - **Broadcast-WebSocketData required** - Any file with WebSocket clients must have this function defined
  - **Port bounds checking** - Always validate port doesn't exceed 65535
  - **Static vs dynamic data** - Pagefile, PSProfiles are static - cache them; counters are dynamic - collect each cycle
  - **EngineEvent placement** - Register-EngineEvent must be before the main loop, not after
  - **CORS restrictions** - Never use wildcard "*" for localhost diagnostics
  - **Process comparison source** - Use correct data source (snapshot.top_ram, not summary)
  ...
  ## 13. Complete Issue Checklist (All Fixed Issues)

  ### Security Issues (Fixed)
  - [x] XSS in debug panel - escape error messages with esc()
  - [x] CSRF token unused - removed or implement properly
  - [x] CORS wildcard - use localhost only, never "*"
  - [x] XSS in HTML reports - use ConvertTo-HtmlEscaped
  - [x] Protected process check on Resume-ProcessById
  - [x] Confirmation header check on all process actions
  - [x] Path traversal in snapshots - validate IDs
  - [x] HTTP method validation on all endpoints

  ### Backend Architecture Issues (Fixed)
  - [x] $refreshRateFile variable order - declare before timer
  - [x] $procId vs $pid conflict - use $procId
  - [x] Broadcast-WebSocketData missing - added to http-helpers
  - [x] Port overflow check - max 65535
  - [x] File locking for cache reads - use retry loops
  - [x] WebSocket cleanup with @() snapshot
  - [x] Empty catch blocks - add console.log
  - [x] EngineEvent placement - before main loop

  ### Metrics & Collectors Issues (Fixed)
  - [x] Memory leak in CommandLines - clear before populating
  - [x] pages/sec complex fallback - keep simple
  - [x] Process group matching - use -contains not regex
  - [x] GPU engine types - track all 8 types
  - [x] Counter availability flag - track and expose
  - [x] Static vs dynamic data - cache pagefile/profiles
  - [x] DriveType in CIM properties
  - [x] Process comparison source - use top_ram not summary

  ### Frontend Issues (Fixed)
  - [x] NaN handling - use (d.value || 0) for undefined
...
### API Issues (Fixed)
- [x] HTTP method validation - return 405 if wrong method
- [x] Request body validation on snapshots
- [x] Numeric parameter parsing for PIDs
- [x] /api/config GET endpoint missing

  ### Documentation Issues (Fixed)
  - [x] README vs code alignment
  - [x] Refresh rate minimum (500ms not 1000ms)
  - [x] AGENTS.md anti-regression rules
  - [x] Version tracking
